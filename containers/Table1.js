...
class TableExampleComplex extends Component {
  ...
  handleStatusItemStyle = status => {
    ...
    if (status === "Out") {
      return <ToggleBlank style={{backgroundColor:'red'}}/>;
    } else if (status === "?") {
      return <ToggleIndeterminate/>;
    } else if (status === "In") {
      return <ToggleCheck style={{backgroundColor:'green'}}/>;
    }
  };
      ...
      <PageBase title="" navigation="BSCC portal / In and out app">
        ...
        <List>
          ...
          {tableData.map((row, index) =>
              <ListItem style={{backgroundColor:'#C7CBC9', margin:5}} primaryText={row.name}  secondaryText={<TextFieldExampleControlled/>} leftIcon={this.handleStatusItemStyle(row.status)} />
      )}
    </List>
